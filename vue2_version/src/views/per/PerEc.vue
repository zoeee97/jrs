<template>
  <div>
    <div>
      <el-input size="small" v-model="jl.eid" placeholder="奖罚员工编号..."
                prefix-icon="el-icon-plus" style="width: 150px"></el-input>
      <el-input size="small" v-model="jl.ecReason" placeholder="奖罚原因..."
...
export default {
  ...
  methods: {
    ...
    // 删除奖罚信息
    deleteHandle(data) {
      this.$confirm('此操作将永久删除员工编号为[' + data.eid + ']的奖惩信息, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
